...
void rk_i2s_suspend(void)
{
	rk30_mux_api_set(GPIO0B0_I2S8CHCLK_NAME, GPIO0B_GPIO0B0);
	gpio_request(RK30_PIN0_PB0, "i2s0_mclk");
	gpio_direction_output(RK30_PIN0_PB0, GPIO_LOW);
}
void rk_i2s_resume(void)
{
	gpio_free(RK30_PIN0_PB1);
	rk30_mux_api_set(GPIO0B0_I2S8CHCLK_NAME, GPIO0B_I2S_8CH_CLK);
}
#ifdef CONFIG_PM
int rockchip_i2s_suspend(struct snd_soc_dai *dai)
{
	I2S_DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);
	return 0;
}

int rockchip_i2s_resume(struct snd_soc_dai *dai)
{
	I2S_DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);
	return 0;
}		
