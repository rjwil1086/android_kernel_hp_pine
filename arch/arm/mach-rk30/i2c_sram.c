...
#define GRF_GPIO_IOMUX GRF_GPIO1L_IOMUX//0xd4 //GRF_GPIO2D_IOMUX
/*ch=0:GRF_GPIO2L_IOMUX,ch=1:GRF_GPIO1L_IOMUX,ch=2:GRF_GPIO5H_IOMUX,ch=3:GRF_GPIO2L_IOMUX*/
#define I2C_GRF_GPIO_IOMUX ((~(0x03<<14))&(~(0x03<<12))) |(0x01<<14) |(0x01<<12)//(0x01<<14)|(0x01<<12)
...
#if 0	
	slavereg = 0x20;
	vccio_voltage = sram_i2c_read(slaveaddr, slavereg);
	data = vccio_voltage & ~(0x3<<2);
	data |= (0x3<<2); //set vio 0x2: 2.5v; 0x3 3.3v
	sram_i2c_write(slaveaddr, slavereg, data);
#endif
...
void __sramfunc rk30_suspend_voltage_resume(unsigned int vol)
{
	...
	if ((arm_voltage > 0x3b)||(arm_voltage < 0x1f))
		data = 0x3b;
	else
		data = arm_voltage;
...
#if 0
	slavereg = 0x20;
	sram_i2c_write(slaveaddr, slavereg, vccio_voltage);
#endif
	slavereg = 0x22;
	sram_i2c_write(slaveaddr, slavereg, data);
...
#else
void __sramfunc rk30_suspend_voltage_set(unsigned int vol){return;}
void __sramfunc rk30_suspend_voltage_resume(unsigned int vol){return;}
#endif
