...
extern void rk30_pm_power_hold(int en);

int tps65910_pre_init(struct tps65910 *tps65910){

	int val = 0;
	int err = -1;
	int reboot_mode = board_boot_mode();
	printk("%s,line=%d,reboot_mode = %d\n", __func__,__LINE__,reboot_mode);
	//gpio_request(PMU_POWER_SLEEP, "NULL");
	//gpio_direction_output(PMU_POWER_SLEEP, GPIO_HIGH);
	
	val = tps65910_reg_read(tps65910,TPS65910_INT_STS);
	printk(KERN_INFO "TPS65910_INT_STS is 0x%x\n", val);
    if(((val&0x4)==0)&&(reboot_mode != BOOT_MODE_REBOOT)&&(reboot_mode != BOOT_MODE_RECOVERY))
	...
	val &= 0;
  #ifndef CONFIG_MACH_RK30_T7H_DVT
	val |= 0x02;
  #endif
	...
	val |= 0x0b;//0x09; turned off vaux1, vdig1,vmmc
   #else
    val |= 0x09; //turned off vaux1, vmmc
   #endif
...
int tps65910_post_init(struct tps65910 *tps65910)
{
	...
	ldo = regulator_get(NULL, "vdig1");	//vcc18_camera
	...
	ldo = regulator_get(NULL, "vdac"); // vcc_18
	regulator_set_voltage(ldo,1800000,1800000);
	regulator_enable(ldo); 
