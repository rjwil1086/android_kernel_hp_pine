...
__weak int rk_lowpower_check(void)
{
	return 0;
}
...
static int  __init start_charge_logo_display(void)
{
	...
	printk("start_charge_logo_display reboot mode = %d\n",board_boot_mode());
	...
	if(val_status.intval == POWER_SUPPLY_STATUS_CHARGING)
	{
		if(rk_lowpower_check()  //low power
           ||(board_boot_mode() != BOOT_MODE_REBOOT))   //do not enter power on charge mode when soft  reset
	    {
