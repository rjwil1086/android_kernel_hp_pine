...
static int marg_c = (int)ARG_C;
static int marg_a = (int)ARG_A;
...
//BL_CORE_DRIVER2 is the flag if backlight is into early_suspend.
    ...
    if(marg_c){
        if(brightness){
         	...
         	* add by hxm
         	*/
         	brightness = (int)(marg_a*bl->props.brightness*bl->props.brightness/100000)+marg_c;
        }
        if(brightness > 255)
            brightness = 255;
    }
...
__weak int rk_battery_notifier_call_chain(unsigned long val)
{
	return 0;
}
#endif
...
#ifdef CONFIG_LOGO_LOWERPOWER_WARNING
	rk_battery_notifier_call_chain(BACKLIGHT_ON);
#endif
