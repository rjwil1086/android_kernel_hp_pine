...
#ifdef RK29_SDCARD_PRESENT
static ssize_t rk29_sdmmc_present_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
    struct platform_device	*pdev = container_of(dev, struct platform_device, dev);
    struct mmc_host *mmc = platform_get_drvdata(pdev);
    return sprintf(buf, "%d\n", rk29_sdmmc_get_cd(mmc));
}

static struct device_attribute  sdmmc_present_attrs[] = {
    __ATTR(present, S_IRUGO | S_IWUSR, rk29_sdmmc_present_show, NULL),
};

static int  rk29_sdmmc_create_sysfs( struct platform_device *pdev )
{
    int r;
    int t;
    for (t = 0; t < ARRAY_SIZE(sdmmc_present_attrs); t++)
    {
        r = device_create_file(&pdev->dev,&sdmmc_present_attrs[t]);
        if (r)
        {
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
   ...
   if(RK29_CTRL_SDMMC_ID == host->pdev->id)   
	...
	printk(KERN_DEBUG "%s,%d.The End of SDMMC-probe %s.  [%s]\n",__FUNCTION__, __LINE__,
	                             RK29_SDMMC_VERSION,host->dma_name);
#if defined(CONFIG_SDMMC0_RK29_SDCARD_DET_FROM_GPIO)
    if(!level_value){
        det_keys_isr(host->gpio_irq,host);
    }
#endif
	return 0;
