...
static ssize_t vflash_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
    int i= 0;
    char *tempBuf = NULL;
    if((vflashData[0]==0) && (vflashData[1]==0))
    {
        tempBuf = (char *)kmalloc(512, GFP_KERNEL);
        if(tempBuf!=NULL)
        {
            GetSNSectorInfo(tempBuf);
            for(i=0; i<512; i++)
    ...
    //len = sprintf(buf, "%d\n", tempBuf);
    return 512;//sprintf(buf, "%d\n", vflashData);
}

static ssize_t model_num_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
    int i= 0;
    char *tempBuf = NULL;
    ...
    for(i=64; i<80; i++)
    {
        //printk("vflashData[%d]=%d\n", i, vflashData[i]);
        tbuf[i-64] = vflashData[i];
    }
    return  sprintf(buf, "%s", tbuf);
}

static struct device_attribute  vflash_attrs[] = {
    __ATTR(sector3_data, S_IRUGO, vflash_show, NULL),
    __ATTR(model_num, S_IRUGO, model_num_show, NULL),     
};

static int  vflash_create_sysfs(struct device *dev)
{
    int r;
    int t;
    for (t = 0; t < ARRAY_SIZE(vflash_attrs); t++)
    {
        r = device_create_file(dev,&vflash_attrs[t]);
        if (r)
        {
...
static int vflash_init(void)
{
    ...
    ret = vflash_create_sysfs(vflash_miscdev.this_device);
	if(ret)
	{
