			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	struct calibrate_offset sense_offset = {0};
	char rate;
	int result = 0;
	...
	case GSENSOR_IOCTL_CALIBRATE:
		if(sensor->ops->auto_calibration){
			if( (sensor->ops->auto_calibration(client,&sense_offset) ) < 0) {
				printk("failed to auto calibration.\n");
				return -EFAULT;
			}
			if ( copy_to_user(argp, &sense_offset, sizeof(sense_offset) ) ) {
				printk("failed to copy sense calibrate data to user space.");
				return -EFAULT;
			}
		}
		break;
	case GSENSOR_IOCTL_SET_CALIBRATE_OFFSET:
		if(sensor->ops->set_calibration){
			if(argp==NULL){
				printk("ERR:	sense calibrate data is NULL.");
			}
			if (copy_from_user(&sense_offset, argp, sizeof(sense_offset)))
				return -EFAULT;
			if(sensor->ops->set_calibration(client,&sense_offset)<0){
				printk("failed to set calibration.\n");
				return -EFAULT;
			}
		}
		break;
