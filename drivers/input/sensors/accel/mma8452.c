...
static int sensor_init(struct i2c_client *client)
{
	...
	ret = sensor_write_reg(client,MMA8452_REG_CTRL_REG5,0xfe);   //switch to int2 for byd t7h
...
static int mma8452_set_calibration(struct i2c_client *client,struct calibrate_offset *offset)
{
	int ret=-1;
	ret=sensor_active(client,0,0);
	ret=sensor_write_reg(client,MMA8452_REG_OFF_X,offset->offsetx);
	ret=sensor_write_reg(client,MMA8452_REG_OFF_Y,offset->offsety);
	ret=sensor_write_reg(client,MMA8452_REG_OFF_Z,offset->offsetz);
	ret=sensor_active(client,1,0);
	if(ret<0){
		DBG("ERR!!!:	%s : failed to set calibration offset\n",__FUNCTION__);
		return ret;
	}

	DBG("%s : set calibration offset successfully\n",__FUNCTION__);
	return 0;

}
static int mma8452_auto_calibration(struct i2c_client *client,struct calibrate_offset *offset)
{
	char buffer[6];
	char tmp=0;
	char regbak[3];
...
*/
	regbak[0]=sensor_read_reg(client,MMA8452_REG_CTRL_REG1);
	tmp=((MMA8452_ASLP_RATE_50)<<MMA8452_ASLP_RATE_SHIFT)|(( MMA8452_ASLP_RATE_1P56)<<MMA8452_RATE_SHIFT)| 4;
		sensor_write_reg(client,MMA8452_REG_CTRL_REG1,tmp);

	regbak[1]=sensor_read_reg(client,MMA8452_REG_XYZ_DATA_CFG);
	tmp=0;
		sensor_write_reg(client,MMA8452_REG_XYZ_DATA_CFG,tmp);

	regbak[2]=sensor_read_reg(client,MMA8452_REG_CTRL_REG2);
	tmp=2;
		sensor_write_reg(client,MMA8452_REG_CTRL_REG2,tmp);

/*
** Clear the Calibration Register
*/
	sensor_write_reg(client,MMA8452_REG_OFF_X,0x00);
	sensor_write_reg(client,MMA8452_REG_OFF_Y,0x00);
	sensor_write_reg(client,MMA8452_REG_OFF_Z,0x00);
/*
...
*/
	...
	sensor_rx_data(client, &buffer[0], 6);
	...
	z_value = buffer[4]<<4;
	z_value += buffer[5]>>4;
	z_value= (~z_value +1)&0x0fff;

        DBG("calibration, x:%d, y:%d, z:%d\n",x_value, y_value, z_value);
/*
**   Calculations for the 12-bit MMA8452Q device calibration
*/
	if ((x_value>>4)> 0x7F)	{
		x_value= (~x_value +1)&0x0fff;
		X_cal= x_value / 2;
	}
	else{
		X_cal= (~x_value+1) / 2;
	...
	if ((z_value>>4)> 0x7F){
//(z_value < 0), should not be here.z_value must be around 1024.
		z_value= (~z_value +1)&0x0fff;
		Z_cal=(1024 + z_value) / 2;
	}
	else{

//mod by cjf
                int new_z_value = z_value - 1024;

                if ((new_z_value>>4)> 0x7F){
                    new_z_value= (~new_z_value +1)&0x0fff;
                    Z_cal= new_z_value / 2;
                }
                else{
                    Z_cal= (~new_z_value+1) / 2;
                }
	/*	Z_cal=(int)(1024- z_value) / 2;
		if (Z_cal<0){
			Z_cal+=256;
		}*/
	}
...
//add by cjf
#define SIGNED_CHAR_OVER_FLOW(value) (value < -128 || value > 127)

        DBG("calibration, xoff:%d, yoff:%d, zoff:%d\n",X_cal, Y_cal, Z_cal);

	if(SIGNED_CHAR_OVER_FLOW(X_cal)){
		DBG("ERR:	%s : X out of calibration:%d\n",__FUNCTION__, X_cal);
		goto err;
	}
	if(SIGNED_CHAR_OVER_FLOW(Y_cal)){
		DBG("ERR:	%s : Y out of calibration:%d\n",__FUNCTION__, Y_cal);
		goto err;
	}
	if(SIGNED_CHAR_OVER_FLOW(Z_cal)){
		DBG("ERR:	%s : Z out of calibration:%d\n",__FUNCTION__, Z_cal);
		goto err;
	}

	Z_cal= (~Z_cal +1)&0x00ff;
/*
** Go to Standby mode and write the Calibration values into the registers
*/
	sensor_active(client,0,0);
	sensor_write_reg(client,MMA8452_REG_OFF_X,(unsigned char)X_cal);
	sensor_write_reg(client,MMA8452_REG_OFF_Y,(unsigned char)Y_cal);
	sensor_write_reg(client,MMA8452_REG_OFF_Z,(unsigned char)Z_cal);
	offset->offsetx=(unsigned char)X_cal;
	offset->offsety=(unsigned char)Y_cal;
	offset->offsetz=(unsigned char)Z_cal;

	sensor_write_reg(client,MMA8452_REG_CTRL_REG1,regbak[0]);
	sensor_write_reg(client,MMA8452_REG_XYZ_DATA_CFG,regbak[1]);
	sensor_write_reg(client,MMA8452_REG_CTRL_REG2,regbak[2]);
	sensor_active(client,1,0);
	//printk("MMA8452_REG_CTRL_REG1=%x \n",regbak[0]);
	//printk("MMA8452_REG_XYZ_DATA_CFG=%x \n",regbak[1]);
	//printk("MMA8452_REG_CTRL_REG2=%x \n",regbak[2]);
//printk("gsensor_auto_calibration---------->xoffset=%x yoffset=%x zoffset=%x\n",X_cal, Y_cal, Z_cal);
//printk("gsensor_auto_calibration---------->(unsigned char) xoffset=%d yoffset=%d zoffset=%d\n",(unsigned char)X_cal, (unsigned char)Y_cal, (unsigned char)Z_cal);

/*
buffer[0] = MMA8452_REG_X_OUT_MSB;
sensor_rx_data(client, &buffer[0], 3);
x_value = buffer[0];
y_value = buffer[1];
z_value = buffer[2];


printk("gsensor_auto_calibration----------->x=%d y=0x%d z=%d\n",(signed short)x_value, (signed short)y_value, (signed short)z_value);
*/
	DBG("%s : auto calibration successfully\n",__FUNCTION__);
...
err:
	...
	sensor_write_reg(client,MMA8452_REG_CTRL_REG2,regbak[2]);
	sensor_active(client,1,0);
	DBG("ERR:	%s : auto calibration fail\n",__FUNCTION__);
	return -EFAULT;
...
struct sensor_operate gsensor_mma8452_ops = {
	...
	.auto_calibration 		= mma8452_auto_calibration,
	.set_calibration 		= mma8452_set_calibration,
};
