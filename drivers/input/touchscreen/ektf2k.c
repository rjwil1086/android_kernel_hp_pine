			...
			const struct i2c_device_id *id)
{
   ...
   // check input_register_devices & input_set_abs_params sequence
	...
	if (err) {
		    printk("[elan]%s: unable to register %s input device\n",
...
int elan_ktf2k_ts_entry(void)
{
    return i2c_add_driver(&ektf2k_ts_driver);
}
EXPORT_SYMBOL_GPL(elan_ktf2k_ts_entry);

void elan_ktf2k_ts_quit(void)
{
    i2c_del_driver(&ektf2k_ts_driver);
}
EXPORT_SYMBOL_GPL(elan_ktf2k_ts_quit);
#else
