...
static void keys_timer(unsigned long _data)
{
	...
	if(state)
		mod_timer(&bdata->timer,
			jiffies + msecs_to_jiffies(button->debounce_msecs));
	else
...
static irqreturn_t keys_isr(int irq, void *dev_id)
{
	...
	BUG_ON(irq != gpio_to_irq(button->gpio));
		...
		input_event(input, type, button->code, 0);
		input_sync(input);
		bdata->ddata->in_suspend = false;
	       return IRQ_HANDLED;
        }
	bdata->long_press_count = 0;

	if (!bdata->key_pressed) {
		bdata->state = button->active_low;
		key_dbg(bdata, "wakeup: %skey[%s]: report ev[%d] state[%d]\n", 
			(button->gpio == INVALID_GPIO)?"ad":"io", button->desc, button->code, bdata->state);
		input_event(input, type, button->code, 1);
		input_sync(input);
...
static int __devinit keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if(button->gpio != INVALID_GPIO) {
			...
			if (!button->debounce_msecs)
				button->debounce_msecs = DEFAULT_DEBOUNCE_INTERVAL;
