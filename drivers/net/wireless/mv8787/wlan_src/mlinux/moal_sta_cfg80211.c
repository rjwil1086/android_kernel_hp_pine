...
/* titan ++ unlock scan wake_lock */        
    //PRINTM(MERROR, "woal_get_scan: wake_unlock\n");
...
/* titan ++ set scan wake_lock*/   
    //PRINTM(MERROR, "woal_set_scan: wake_lock\n");
    wake_lock(&priv->phandle->scan_lock);
/* titan -- */
#endif
...
{
    PRINTM(MERROR, "[Awin debug driver] woal_cfg80211_init_wiphy\n");
    mlan_status ret = MLAN_STATUS_SUCCESS;
    ...
    cfg_11n->param.tx_cfg.httxcap = 0x62;
    PRINTM(MERROR, "[Awin debug driver] woal_cfg80211_init_wiphy httxcap : %x\n", cfg_11n->param.tx_cfg.httxcap);
    ...
    cfg_11n->sub_command = MLAN_OID_11N_HTCAP_CFG;
    req->req_id = MLAN_IOCTL_11N_CFG;
    cfg_11n->param.htcap_cfg.htcap = 0x1820000;
    ...
    //disable robust
    //woal_host_command_disable_robust(priv);

#ifdef TX_POWER_TO_SAR
    //Set tx power to 13dBm
    woal_host_command_set_txpower_to_sar(priv);
#endif
    //modify by Azw --
