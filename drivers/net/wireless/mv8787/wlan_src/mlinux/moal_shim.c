...
//titan msleep
t_void 
moal_msleep(IN t_void * pmoal_handle, IN t_u32 msec)
{
	msleep(msec);
}
//titan msleep
BOOLEAN
moal_set_get_first_wakeup_card(IN t_void * pmoal_handle, IN BOOLEAN action, IN BOOLEAN value)
{
	moal_handle *handle = (moal_handle *) pmoal_handle;
	if(action == 0)
		return handle->first_wakeup_card;
	else{
		handle->first_wakeup_card = value;
		return MTRUE;
	}
}
/** 
...
//Neo Modify
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,8) || defined(COMPAT_WIRELESS)
