...
woal_sdio_suspend(struct device *dev)
{
    ...
    //printk("<--- Enter woal_sdio_suspend --->\n"); //titan
    ...
    //printk( "<--- Leave woal_sdio_suspend --->\n"); //titan
    LEAVE();
    return ret;
}
...
woal_sdio_resume(struct device *dev)
{
    ...
    //printk( "<--- Leave woal_sdio_resume --->\n"); //titan
    handle->first_wakeup_card = MTRUE; //titan msleep
    LEAVE();
    return MLAN_STATUS_SUCCESS;
}
#endif
