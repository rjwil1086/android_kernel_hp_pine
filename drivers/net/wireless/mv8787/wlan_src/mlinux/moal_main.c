...
static mlan_callbacks woal_callbacks = {
    ...
    .moal_msleep = moal_msleep, //titan msleep
    .moal_set_get_first_wakeup_card = moal_set_get_first_wakeup_card, //titan msleep
...
extern struct platform_driver wifi_driver;
#endif
/** Default Driver mode */
...
#ifdef ANDROID
		//titan
		/* scan / connet wake-lock */
    wake_lock_init(&handle->scan_lock, WAKE_LOCK_SUSPEND, "scan_lock");
    wake_lock_init(&handle->assoc_lock, WAKE_LOCK_SUSPEND, "assoc_lock"); 
    wake_lock_init(&handle->disassoc_lock, WAKE_LOCK_SUSPEND, "disassoc_lock");
#endif
...
#endif
#endif
		handle->first_wakeup_card = MFALSE; //titan msleep
    /* Register to MLAN */
...
#ifdef ANDROID    
    /* titan: destroy wake lock */
    wake_lock_destroy(&handle->scan_lock);
    wake_lock_destroy(&handle->assoc_lock);    
    wake_lock_destroy(&handle->disassoc_lock); 
#endif   
