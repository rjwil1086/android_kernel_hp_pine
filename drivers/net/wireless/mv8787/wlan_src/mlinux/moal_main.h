...
#ifdef ANDROID
#include 	<linux/wakelock.h> //titan
#endif
...
struct _moal_handle
{
    ...
    t_s8 driver_version[MLAN_MAX_VER_STR_LEN];
		...
		struct wake_lock assoc_lock;
		struct wake_lock disassoc_lock;		
#endif
		BOOLEAN	first_wakeup_card; //titan msleep
};
...
int woal_host_command_disable_robust(moal_private * priv);
int woal_host_command_enable_robust(moal_private * priv);
int woal_host_command_set_txpower_to_sar(moal_private * priv);
#endif
