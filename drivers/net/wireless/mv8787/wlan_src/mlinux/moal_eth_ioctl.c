...
#define CMD_BUF_LEN   2048
	...
	printk("[TITAN] receive a START command\r\n");
 	woal_send_iwevcustom_event(priv, CUS_EVT_START); //titan
        len = sprintf(buf, "OK\n") + 1;
    } else if (strncmp(buf, "STOP", strlen("STOP")) == 0) {
	printk("[TITAN] receive a STOP command\r\n");
 	//printk("[TITAN] receive a STOP command, do disconnect proceudre\n");
	woal_disconnect(priv, MOAL_IOCTL_WAIT, NULL);	//titan
	woal_send_iwevcustom_event(priv, CUS_EVT_STOP);    	//titan
        len = sprintf(buf, "OK\n") + 1;
	msleep(1000);
	printk("[TITAN]send STOP command back to supplicant");
	woal_send_iwevcustom_event(priv, CUS_EVT_STOP);       //titan
	printk("[TITAN] Exit STOP command procedure\n");  
    }
#ifdef UAP_SUPPORT
