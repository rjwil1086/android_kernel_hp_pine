...
static region_code_mapping_t region_code_mapping[] = {
    ...
    {"HP ", 0xaa},            //Special for HP ,add for king.xu 2013.08.05
};
#endif
...
/* titan ++ set connect wake_lock */
    //PRINTM(MMSG, "woal_disconnect: wake_lock\n");
    wake_lock(&priv->phandle->disassoc_lock);
/* titan -- */
#endif
    /* Allocate an IOCTL request buffer */
    req = (mlan_ioctl_req *) woal_alloc_mlan_ioctl_req(sizeof(mlan_ds_bss));
...
/* titan ++ unlock connect wake_lock */
    //PRINTM(MMSG, "woal_disconnect: wake_unlock\n");
    wake_unlock(&priv->phandle->disassoc_lock);
/* titan -- */
#endif  
    LEAVE();
    return status;
...
/* titan ++ set connect wake_lock */
    ...
    wake_lock(&priv->phandle->assoc_lock);
/* titan -- */
#endif   
...
/* titan ++ unlock connect wake_lock */
    ...
    wake_unlock(&priv->phandle->assoc_lock);
/* titan -- */
#endif        
    LEAVE();
    return status;
...
woal_host_command(moal_private * priv, struct iwreq *wrq)
{
    ...
    mlan_ioctl_req *req = NULL;
    int i;
    t_u8* temp = (t_u8*)wrq->u.data.pointer;
    ...
    //PRINTM(MERROR, "[Awin debug] Host command len = %u\n", misc->param.hostcmd.len);
    //for(i = 0 ; i < 404 ; i ++){
    //    PRINTM(MERROR, "Host command = 0x%02x  ,%u\n", *(temp+i), i);
    //}   
    //PRINTM(MERROR, "[Awin debug] Copy cmd : \n");
    //for(i = 0 ; i < 404 ; i ++){
...
int
woal_host_command_disable_robust(moal_private * priv)//, struct iwreq *wrq)
{
    HostCmd_Header cmd_header;
    int ret = 0;
    mlan_ds_misc_cfg *misc = NULL;
    mlan_ioctl_req *req = NULL;
    unsigned char robust_temp[20] = {0xe0, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                                     0x00, 0x00, 0x60, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    /* get command header */
    if (!memcpy
        (&cmd_header, robust_temp, sizeof(HostCmd_Header))) {
        PRINTM(MERROR, "copy from user failed: Host command header\n");
        ret = -EFAULT;
        goto done;
    }
    ...
    PRINTM(MERROR, "Host command len = %u\n", misc->param.hostcmd.len);
    if (!misc->param.hostcmd.len ||
        misc->param.hostcmd.len > MLAN_SIZE_OF_CMD_BUFFER) {
        PRINTM(MERROR, "Invalid data buffer length\n");
        ret = -EINVAL;
    ...
    /* get the whole command from user */
    if (!memcpy
        (misc->param.hostcmd.cmd, robust_temp,
         woal_le16_to_cpu(cmd_header.size))) {
        PRINTM(MERROR, "copy from user failed\n");
        ret = -EFAULT;
        goto done;
    ...
    if (MLAN_STATUS_SUCCESS != woal_request_ioctl(priv, req, MOAL_IOCTL_WAIT)) {
        ...
        PRINTM(MERROR, "woal_host_command_disable_robust succ\n");
    }
...
int
woal_host_command_set_txpower_to_sar(moal_private * priv)//, struct iwreq *wrq)
{
    HostCmd_Header cmd_header;
    int ret = 0;
    mlan_ds_misc_cfg *misc = NULL;
    mlan_ioctl_req *req = NULL;
    unsigned char robust_temp[404] = {
      };
    ...
    if (MLAN_STATUS_SUCCESS != woal_request_ioctl(priv, req, MOAL_IOCTL_WAIT)) {
        ...
        PRINTM(MERROR, "woal_host_command_set_txpower_to_sar succ\n");
    }
...
int
woal_host_command_enable_robust(moal_private * priv)//, struct iwreq *wrq)
{
    HostCmd_Header cmd_header;
    int ret = 0;
    mlan_ds_misc_cfg *misc = NULL;
    mlan_ioctl_req *req = NULL;
    //use Marvell default RBC
    unsigned char robust_temp[36] = {0xe0, 0x00, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                                     0x00, 0x00, 0x60, 0x01, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00,
                                     0x61, 0x01, 0x0c, 0x00, 0x02, 0x00, 0x00, 0x00, 0x30, 0x75, 
                                     0x10, 0x27, 0x50, 0xc3, 0x00, 0x00};
    ENTER();
    ...
    misc->sub_command = MLAN_OID_MISC_HOST_CMD;
    req->req_id = MLAN_IOCTL_MISC_CFG;
    if (MLAN_STATUS_PENDING != woal_request_ioctl(priv, req, MOAL_NO_WAIT)) {
        ret = -EFAULT;
        goto done;
    }else{
        PRINTM(MERROR, "woal_host_command_enable_robust succ\n");
    }
