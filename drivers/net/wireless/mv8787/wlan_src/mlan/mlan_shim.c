...
mlan_main_process(IN t_void * pmlan_adapter)
{
    ...
    mlan_adapter *pmadapter = (mlan_adapter *) pmlan_adapter;
    pmlan_callbacks pcb;
		t_bool first_wakeup_card = MFALSE; //titan msleep
  ...
  process_start:
    do {
        ...
        pcb->moal_spin_unlock(pmadapter->pmoal_handle,
                              pmadapter->pmain_proc_lock);
        //titan msleep ++
        first_wakeup_card = pcb->moal_set_get_first_wakeup_card(pmadapter->pmoal_handle,0, 0);
        if(first_wakeup_card == MTRUE){
		PRINTM(MERROR,"Sleep 500ms\r\n");
        	pcb->moal_msleep(pmadapter->pmoal_handle,500);
	}
        //titan msleep --
        ...
        //titan msleep ++
        first_wakeup_card = pcb->moal_set_get_first_wakeup_card(pmadapter->pmoal_handle,
        																												0, 0);
        if(first_wakeup_card == MTRUE)
        {
        	PRINTM(MERROR," no one wakeup card, wakeup it once\r\n");
          pmadapter->pm_wakeup_fw_try = MTRUE;
          wlan_pm_wakeup_card(pmadapter);
          continue;        	
        }
